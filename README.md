...
`trellis-zero-to-mvp-zh` 和英文版 `trellis-zero-to-mvp` 都会在规划阶段运行自我评审循环，先选择 Project Contract Profile，按 Trellis 0.6+ 产物边界生成 `prd.md`、`design.md`、`implement.md` 和必要 JSONL。它重点检查需求覆盖、契约一致性、占位符、验收断言、复杂任务拆分、批次完整性和 Artifact Gate。确认完整规划后，再让它创建 Trellis 任务树。

对于 Codex 项目，它会读取 `.trellis/config.yaml` 的 `codex.dispatch_mode`：`inline` 模式下 JSONL 不作为规划就绪门槛；sub-agent 或明确需要稳定上下文预加载时，JSONL 必须填真实条目，不能保留 `_example` 种子。

中文版 `trellis-zero-to-mvp-zh` 还支持 Design Surface Gate：子任务 PRD 会声明是否涉及数据库、API、模块间交互、外部系统、UI 风格、权限、状态、校验、事务、测试等设计面；任一影响面为“是”时，必须在 `design.md` 和 `implement.md` 补齐对应章节，否则 Artifact Gate 失败。这用于约束小模型在 RuoYi/Vue 等复杂后台项目中不能跳过表结构、接口契约、页面风格和权限设计。
...
**技能会做什么**：
- 读取子任务的 `prd.md`（需求、验收标准、行为约束、依赖和契约引用）
- 如有 `design.md`，读取技术设计、编排-计算分离和挂载点清单
- 如有 `implement.md`，读取文件计划、有序步骤、自检命令、失败恢复和评审门
- 为每条验收标准（AC-001, AC-002...）执行红绿循环：
  1. **RED**：写一个失败的测试（照抄 `prd.md` 参考实现的测试范例）
  2. **看红**：运行测试，必须看到失败（没失败 = 测试没覆盖行为，回去修测试）
  3. **GREEN**：写刚好让它变绿的最小代码，落点按 `implement.md` 文件计划和 `design.md` 分层
  4. **看绿**：再次运行测试，看到通过
  5. **自检**：运行 `implement.md` 中的全部自检命令；低复杂度 PRD-only 任务运行 PRD 精简执行附录中的命令，确认无回归
  6. **记录**：标记 AC 为 done，暂存（不 commit），进入下一条 AC

**关键约束**：
- 没有失败测试，不写实现代码
- 一次只让一条 AC 变绿
- 不动 `implement.md` 文件计划/禁止事项之外的文件；低复杂度 PRD-only 任务按 PRD 精简执行附录执行
...
本技能集仍兼容 Trellis 的核心任务结构（`.trellis/tasks/`、`.trellis/spec/`、`task.py create --parent`），同时增加对 0.6+ 工作流的适配：
...
- 对中/高复杂度任务，除 `prd.md` 外，按项目工作流补充 `design.md`、`implement.md`、`implement.jsonl`、`check.jsonl`；PRD 只承载需求、约束、范围、依赖和验收，执行步骤放入 `implement.md`。
- 对 Codex 项目读取 `.trellis/config.yaml` 的 `codex.dispatch_mode`；inline 模式下 JSONL 不作为规划就绪门槛，sub-agent 模式下 JSONL 必须填真实上下文。
...
│   ├── references/
│       ├── workflow-state-machine.md     # 状态机与合法跳转
│       ├── gate-definitions.md           # Gate 与失败码定义
│       ├── project-contract-profiles.md  # 项目契约画像
│       ├── small-model-safety.md         # 小模型长程规划防漂移
│       ├── subagent-planning-template.md # 子代理/批次规划模板
│       ├── analysis-output-template.md   # 只读分析输出模板
│       ├── parent-prd-template.md        # 父任务 PRD 模板
│       ├── child-prd-template.md         # 子任务 PRD 模板
│       ├── planning-artifacts-template.md # 0.6+ 设计/实现/上下文清单模板
│       ├── self-review-checklist.md      # 自我评审检查清单
│       ├── self-review-report-template.md # 自我评审报告模板
│       └── task-creation-checklist.md    # 任务创建检查清单
│   └── scripts/
│       └── trellis_zero_gate.py           # Artifact Gate 机械扫描
...
│   └── （结构同 trellis-zero-to-mvp，另含 references/design-surface-template.md 设计面模板）
