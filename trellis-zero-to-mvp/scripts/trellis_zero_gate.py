#!/usr/bin/env python3
"""Deterministic artifact gate for trellis-zero-to-mvp outputs.
...
PLACEHOLDER_RE = re.compile(
    ...
    r"to be provided|pending config|depending on situation|as needed|depends"
)

EXTERNAL_CONFIG_RE = re.compile(
    r"YOUR_KEY|API_KEY_HERE|待用户提供|待配置|待申请|待开通|待获取|"
    r"placeholder key|replace with.*key|to be provided|pending config",
    re.IGNORECASE,
)
